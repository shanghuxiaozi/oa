...
<form class="layui-form" action="" target="nm_iframe" id="i_form">
	<blockquote class="layui-elem-quote">
	   	<div class="layui-form-item">
	        <span class="layui-form-label" style="width: 85px;">身份证号码：</span>
...
<form id="downLoadForm" action="/InvitationUser/downloadUserData" method="post">
	...
	<input type="hidden" name="wordUnit" id="workUnit2" />
	<input type="hidden" name="unitAttribute" id="unitAttribute2"/>
	<input type="hidden" name="postNature" id="postNature2"/>
</form>
...
//  layer.msg('Hello World');
	...
	table.render({
		  ...
		  ,response: {
	 	  ...
	 	  ,cols:[[
		       ...
		       ,{field: 'startTime', title: '入职时间', width: 120, align:'center' } 
		       ,{field: 'endTime', title: '离职时间', width: 120, align:'center' }
		       ,{field: 'contractStartTime', title: '合同开始时间', width: 120, align:'center' }
		       ,{field: 'contractEndTime', title: '合同结束时间', width: 120, align:'center' }
		       ,{field: 'basyPay', title: '基本工资', width: 120, align:'center' }
		       ,{field: 'isLeave', title: '是否离职1是0否', width: 120, align:'center' }
		       ,{field: 'isJob', title: '是否在项目中', width: 120, align:'center' }
		       ,{field: 'whichOne', title: '项目', width: 120, align:'center' }
    ...
    $(document).ready(function() {
        ...
        var schoolTerm =["请选择","机关单位","事业单位","企业"];
        var content=[['请选择'],['公务员', '事业单位职员', '其他'],['公务员', '事业单位职员', '其他'],
            ['企业职工管理岗', '企业职工销售岗', '企业职工技术岗','企业职工生产岗','企业职工研发岗','企业职工综合岗','其他']];
        ...
        for( var i=0; i<schoolTerm.length ;i++){
            ...
            s3.add(new Option( schoolTerm[i],schoolTerm[i]),null);
        }
        //初始化岗位
        s2.innerHTML ="";
        s4.innerHTML ="";
        for( var i=0; i<content[0].length ;i++){
            ...
            s4.add(new Option(content[0][i],content[0][i]),null);
        }
        //此方法一定要加，相当于重新渲染
        form.render();
        //相当于onchange事件
        ...
        form.on("select(state2)",function(data){
            var index = s3.selectedIndex;
            s4.innerHTML ="";
            for( var i=0; i<content[index].length ;i++){
                s4.add(new Option(content[index][i],content[index][i]),null);
            }
            form.render();
        });
    });
	...
	$("#downLoadBtn").on("click",function(){
        ...
        $("#workUnit2").val($.trim($("#unit").val()));
        $("#unitAttribute2").val($.trim($("#s1").val()));
        $("#postNature2").val($.trim($("#s2").val()));
