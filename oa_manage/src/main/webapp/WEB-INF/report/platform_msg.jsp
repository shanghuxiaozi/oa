...
<form class="layui-form layui-form-pane" action="####" style="margin-right: 5%; margin-top: 10px;" id="formperm">
	...
	<div class="layui-form-item" >
		<label class="layui-form-label">留言人名称:</label>
	...
	<div class="layui-form-item" >
		<label class="layui-form-label">留言人昵称:</label>
		<div class="layui-input-block">
	...
	<div class="layui-form-item" pane>
		<label class="layui-form-label">留言截图:</label>
...
<!-- 向用户发送信息页面 -->
<div style="display: none" id="messageForm" >
	<div class="layui-form-item layui-form-text">
        <div class="layui-form-item layui-form-text">
          <label class="layui-form-item"></label>
          <div class="layui-form-item">
            <textarea id="message" placeholder="感谢您提供的意见与建议，特奖励您10积分，希望您一如既往的支持本平台。" class="layui-textarea"></textarea>
          </div>
        </div>
...
layui.use(['layedit', 'jquery','laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function(){
	...
	table.render({
		  ...
		  ,response: {
	 	  ...
	 	  //设置表头  进行数据渲染
	 	  ,cols:[[
	 	        {field: 'uname2', title: '留言人名称', width: 150, align:'center'}
	 	       ,{field: 'unickname2', title: '留言人昵称', width: 150, align:'center'}
	 	       ,{field: 'content', title: '留言内容', width: 320, align:'center'}
//		       ,{field: 'uname', title: '被举报人名称', width: 150, align:'center'}
//		       ,{field: 'unickname', title: '被举报人昵称', width: 150, align:'center'}
		       ,{field: 'time', title: '留言时间', width: 150, align:'center'}
		       ,{field: 'image', title: '留言图片',templet:'#imgTpl',width: 180, align:'center' }
		...
		if(layEvent === 'edit'){
    		...
    		layer.open({
    			...
    			,yes: function(index,layero){
    				if(data.status != '2'){
    					layer.msg('该留言内容已经处理过,不能进行重复操作!',{icon: 2});
    					return false;
    				}
    				// 未处理过的数据才能继续处理.
    				layer.open({
    					...
    					,yes: function(index,layero){
    	    				...
    	    				$.ajax({
    	    					type:"POST",
    	    			        dataType: 'json',
    	    			        url:js_path+'/report/plusIntegral',
    	    				    data:{'id':data.id,'content':content},		
    	    				    success: function(data) {
    	    				    	layer.msg('处理成功!', {icon: 6});
    	    				    	spotAndModuleHandler('4',obj.data.tipOffUserId);
    	    				    	spotAndModuleHandler('9',obj.data.tipOffUserId);
    	    				    }
    	    				});
    	    				layer.closeAll();
    						queryDynamic();
