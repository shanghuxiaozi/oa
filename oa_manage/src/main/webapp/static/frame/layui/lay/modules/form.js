...
layui.define('layer', function(exports){
  ...
  /**批量设置表单值的方法**/
  Form.prototype.setInfo = function(id,data){
          ...
          $(id).find('[name]').each(function() {
              ...
              $(id).find(type+"[name='"+name+"']").val($.trim(data[''+name+'']));
          });
  }
  
  /**为按钮添加tips的方法**/
  Form.prototype.addTips = function(id,msg,type,color,width,time){
	  var addPersonelBtnTips = -1;
      $(id).mouseover(function(){
      	//tips层-右
      	if(addPersonelBtnTips ==-1 )
      	addPersonelBtnTips = layer.tips(msg, id, {
      	  tips: [type||2, color||'#0FA6D8'],
      	  maxWidth: width||210,
      	  time:time||3000
      	});
      });
      ...
      $(id).mouseout(function(){
      	//tips层-右
      	layer.close(addPersonelBtnTips);
      	addPersonelBtnTips = -1;
      });
  }
  
  /**批量拿到表单的值**/
  Form.prototype.getFormparam = function(id){
          var param = {};
          //简便方法
          $(id).find('[name]').each(function() {
        	  //console.log("form--",$(this).attr('type') ,$(this).attr('checked'));
        	  if($(this).attr('type') == 'radio' /*&& $(this).attr('checked') == 'checked'*/)
        		 // param[$(this).attr('name')] = $(this).val();
        	  param[$(this).attr('name')] = $("[name='"+$(this).attr('name')+"']:checked").val();	
  ...
  /**删除form表单的值**/
  Form.prototype.clear = function(id){
	//简便方法
      $(id).find('[name]').each(function() {
          var type = $(this)[0].nodeName.toLowerCase();
          var name = $(this).attr('name');//console.log('type='+type);
          if(type == 'input'&& $(this).attr('type') != 'radio')
          $(id+" "+type+"[name='"+name+"']").val('');
          else if(type == 'select'){
        	  $(id+" "+type+"[name='"+name+"']").find('option').removeAttr('selected');
        	  $($(id+" "+type+"[name='"+name+"']").find('option')[0]).attr('selected');
          }else if($(this).attr('type') == 'checkbox'){
        	  $(id+" "+type+"[name='"+name+"']").removeAttr('checked');
  ...
  Form.prototype.render = function(type){
    var that = this, items = {
      ...
      //单选框
      ,radio: function(){
        var CLASS = 'layui-form-radio', ICON = ['&#xe643;', '&#xe63f;'], ICON1 = ['&#xe627;', '&#xe626;']
        ,radios = $(ELEM).find('input[type=radio]')
        
        ,events = function(reElem){
          var radio = $(this), ANIM = 'layui-anim-scaleSpring';
          var rType = radio.attr('lay-return');
          reElem.on('click', function(){
            ...
            layui.each(sameRadio, function(){
              ...
              this.checked = false;//console.log("循环",this);
              next.removeClass(CLASS+'ed');
              if(rType == "1")
            	  next.find('.layui-icon').removeClass(ANIM).html(ICON1[1]);
              else
            	  next.find('.layui-icon').removeClass(ANIM).html(ICON[1]);
            });
            //$(radio[0]).attr('checked','');
            radio[0].checked = true;//console.log($('input[type="radio"]:checked').val(),radio[0]);
            reElem.addClass(CLASS+'ed');
            if(rType == "1")
            	 reElem.find('.layui-icon').addClass(ANIM).html(ICON1[0]);
            else
        ...
        radios.each(function(index, radio){
          ...
          var rType = $(radio).attr('lay-return');
          var reElem = "";//console.log("rType="+rType);
          //替代元素
          if(rType == "1"){
        	  reElem = $(['<div class="layui-unselect '+ CLASS + (radio.checked ? (' '+CLASS+'ed') : '') + (disabled ? ' layui-radio-disbaled '+DISABLED : '') +'">'
                  ,'<span>'+ (radio.title||'未命名') +'</span>'
                  ,'<i class="layui-anim layui-icon">'+ ICON1[radio.checked ? 0 : 1] +'</i>'
                  ,'</div>'].join(''));
  ...
  /**表单单独验证**/
  Form.prototype.verifyField = function(id){
	  var  verify = form.config.verify, stop = null
	    ,DANGER = 'layui-form-danger', field = {} ,elem = $(id)
	    
	    ,verifyElem = elem.find('*[lay-verify]') //获取需要校验的元素
	    ,formElem = $(id) //获取当前所在的form元素，如果存在的话
	    ,fieldElem = elem.find('input,select,textarea') //获取所有表单域
	    ,filter = 'lay-filter'; //获取过滤器
