...
function getParametersJson(){
	  ...
	  t.locked=$("#locked").val();
	  var sysUserVo=new Object();
	  sysUserVo.page=currentPage - 1;
	  sysUserVo.pageSize=10;
	  sysUserVo.t=t;
	  return JSON.stringify(sysUserVo);
}
...
layui.use(['table','form','element','layer'], function(){
        ...
        var options = {
        	...
        	url:js_path+'/sysUser/queryDynamic',//查询接口
        ...
        function editData(item,row){
        	var layerEdit = layer.open({
                ...
                shade: 0,
                maxmin: true,
                content:js_path+'/sysUser/edit?id='+item.id,
                btn: ['确定', '取消'], //只是为了演示
                yes: function(index,layero){
                	var flag=window["layui-layer-iframe" + index].formSubmit();
                        ...
                        if(dataAdd == null){
                            ...
                            layer.msg('密码不符合规则', {icon: 3});
                            return false;
                        }
                    	$.ajax({
                    		type: "PUT",
                    		dataType: "json",
                    		url: js_path+'/sysUser/update',
                    		data:dataAdd,
      ...
      //删除调用方法
        ...
        function detailData(item,row){
        	var layerEdit = layer.open({
                ...
                shade: 0,
                maxmin: true,
                content:js_path+'/sysUser/detail?id='+item.id,
                btn: ['关闭'],
                yes: function(index,layero){
                	layer.closeAll();
                }
       ...
       $('#edit').click(function(e){
        	...
        	var layerEidt=layer.open({
                 ...
                 shade:['0.3','#000'],
                 maxmin: true,
                 content:js_path+'/sysUser/edit?id='+ids[0],
                 btn: ['确认', '取消'],
                 yes: function(index,layero){
                	var flag=window["layui-layer-iframe" + index].formSubmit();
               	 	if(flag){
       ...
       //查询按钮事件
		...
		function queryDynamic(){
			...
			$.ajax({
				type: "POST",
				url: js_path+'/sysUser/queryDynamic',
