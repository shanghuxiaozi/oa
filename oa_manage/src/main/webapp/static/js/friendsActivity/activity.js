...
layui.use(['table','form','element','layer'], function(){
        ...
        var options = {
        	...
        	url:js_path+'/activity/queryDynamic',//查询接口
       ...
       //编辑调用方法
        function editData(item,row){
        	var layerEdit = layer.open({
                ...
                shade: 0,
                maxmin: true,
                content:js_path+'/activity/edit?id='+item.id,
                btn: ['确定', '取消'], //只是为了演示
      ...
      //删除调用方法
        ...
        function detailData(item,row){
        	var layerEdit = layer.open({
                ...
                shade: 0,
                maxmin: true,
                content:js_path+'/activity/detail?id='+item.id,
                btn: ['关闭'],
                yes: function(index,layero){
                	layer.closeAll();
                }
                 ...
                 yes: function(index,layero){
                	var flag=window["layui-layer-iframe" + index].formSubmit();                	
                    	...
                    	$.ajax({
                    		type: "POST",
                    		dataType: "json",
                    		url: js_path+'/activity/pushMessage',
                    		data:{'json':getEntity(flag)},
                    		success: function(data) {
