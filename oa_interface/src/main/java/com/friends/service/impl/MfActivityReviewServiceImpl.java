...
import com.friends.dao.MfActivityReviewDao;
...
@Service
public class MfActivityReviewServiceImpl extends BaseService<MfActivityReviewDao, MfActivityReview>
		implements MfActivityReviewService {

	@Autowired
	MfActivityReviewDao mfActivityReviewDao;
	@Autowired
	EntityManager em;
	...
	public Result queryDynamic(MfActivityReviewDynamicQueryVo mfActivityReviewDynamicQueryVo) {
		int page = mfActivityReviewDynamicQueryVo.getPage();
		int pageSize = mfActivityReviewDynamicQueryVo.getPageSize();
		MfActivityReview mfActivityReview = mfActivityReviewDynamicQueryVo.getT();

		// 多表sqlCommonBody查询 返回单表数据
		StringBuilder sqlCommonBody = new StringBuilder();
		sqlCommonBody.append(" from mf_activity_review t where 1=1 ");
